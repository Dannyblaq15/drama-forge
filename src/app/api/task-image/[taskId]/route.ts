...
// Polls an async image generation task from DashScope
export async function GET(req: Request, { params }: { params: Promise<{ taskId: string }> }) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: errData.message || 'Failed to poll image task' }, { status: response.status });
    }

    const data = await response.json();
    // DashScope image task shape: data.output.task_status, data.output.results[0].url
